...
<body>
	...
	<div class="gallary-list">
		...
		<%
		// 검색어가 있을 경우, 해당 조건에 맞는 항목만 필터링
		if (query != null && !query.isEmpty()) {
			List<Tour> filteredTours = new ArrayList<>();
			for (Tour t : tourlist) {
				if (t.getTL_NAME().toLowerCase().contains(query.toLowerCase())) {
					filteredTours.add(t);
				}
			}

			// 필터링된 항목들 출력
			for (Tour t : filteredTours) {
				String[] imgarray;
		...
		<div class="gallary-detail" onclick="location.href='tours/<%=t.getTL_NO()%>'">
			<div class="image-box">
				...
				if (t.getTL_IMG() == null || t.getTL_IMG().equals("")) {
				%>
				<img src="<%=imgarray[0]%>" style="width: 100%; object-fit: cover; border-top-right-radius: 7px; border-top-left-radius: 7px;">
				<%
				} else {
				%>
				<img src="<%=imgarray[0] != null ? "touristimg/" + imgarray[0] : "https://placehold.co/250x166/EFEFEF/6D6D6D?text=No+Image"%>" style="width: 100%; object-fit: cover; border-top-right-radius: 7px; border-top-left-radius: 7px;">
				<%
				}
				%>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // searchInput 값이 있을 때 자동으로 엔터를 치도록 처리
        const searchInput = document.getElementById("searchInput").value.trim();

        // 만약 값이 있으면 엔터를 친 것처럼 처리
        if (searchInput.length > 0) {
            // 검색 기능 호출
            search({ preventDefault: () => {} });  // { preventDefault: () => {} } 로 빈 함수 추가하여 event 객체 대체
        }
    });
